...
#endif
{
    ...
    mpCC.reset(new CentralControl(mNh,mNhPrivate,mClientId,mSysState,shared_from_this(),mpUID));//初始化中央控制器
    ...
    else if(mSysState == eSystemState::SERVER)
    {
        ...
        *ss << "FrameId" << mClientId;
        mNhPrivate.param(ss->str(),mpCC->mNativeOdomFrame,std::string("nospec"));
        //cout<<"------------"<<mpCC->mNativeOdomFrame<<endl;
    }
    else
    {
        cout << "\033[1;31m!!!!! ERROR !!!!!\033[0m ClientHandler::InitializeThreads(): invalid systems state: " << mpCC->mSysState << endl;
...
//     mpComm.reset(new Communicator(mpCC,mpVoc,mpMap,mpKFDB,mpPcm));
...
void ClientHandler::InitializeServer(bool bLoadMap)
{
    ...
    mpLoopFinder.reset(new LoopFinder(mpCC,mpKFDB,mpVoc,mpMap,mpViewer->mpPointCloudMapping));
    ...
    usleep(10000);
    //+++++ Initialize the communication thread +++++
    //mpPcm=boost::make_shared<PointCloudMapping>( 0.03,50,2 );
    mpComm.reset(new Communicator(mpCC,mpVoc,mpMap,mpKFDB,mpViewer->mpPointCloudMapping,bLoadMap));
    mpComm->SetMapping(mpMapping);
    usleep(10000);
    mpMapping->SetCommunicator(mpComm);
    mpMap->SetCommunicator(mpComm);
