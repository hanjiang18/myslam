...
LoopFinder::LoopFinder(ccptr pCC, dbptr pDB, vocptr pVoc, mapptr pMap,pcmptr pPointCloud)
    : mpCC(pCC), mpKFDB(pDB), mpVoc(pVoc), mpMap(pMap),
      mbResetRequested(false),mpPointCloudMapping(pPointCloud),
      mbFixScale(true),mKFcount(0),//mbfixscale
...
void LoopFinder::CorrectLoop()
{
    ...
    //cout<<mpCurrentKF->mId.first<<"    "<<mpCurrentKF->GetPose()<<endl;
